    ...
    <main className="min-h-screen hero-gradient text-white">
      <div className="mx-auto max-w-[900px] px-4 py-12 md:py-20">
        ...
        <div className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center text-sm text-gray-400 gap-4">
            ...
            <div className="flex items-center">
              ...
              <span>{intl.t('CodeFormer AI, 图像增强, AI技术')}</span>
            </div>
          </div>
        </div>
        ...
        <article className="prose prose-lg max-w-none prose-invert">
          ...
          <h2>{intl.t('CodeFormer AI的核心技术原理')}</h2>
          <p>
            {intl.t(
          ...
          <ul>
            <li>
              <strong>{intl.t('编码簿机制 (Codebook)：')}</strong>
              {intl.t(
                '系统维护一个由高质量图像特征构成的"词典"，这些特征用于指导低质量图像的修复。'
              )}
            </li>
            <li>
              <strong>{intl.t('Transformer架构：')}</strong>
              {intl.t(
                '利用自注意力机制捕捉图像不同区域之间的长距离依赖关系，实现全局一致的修复效果。'
              )}
            </li>
            <li>
              <strong>{intl.t('双向学习：')}</strong>
              {intl.t(
                '同时进行特征提取和图像生成两个过程，确保修复的图像不仅高清晰，还保持原始图像的身份特征和真实感。'
              )}
            </li>
          </ul>
          ...
          <h2>{intl.t('CodeFormer AI与传统图像增强技术的对比')}</h2>
          <p>
            {intl.t(
          ...
          <ul>
            <li>
              <strong>{intl.t('更智能的内容识别：')}</strong>
              {intl.t(
                '能够理解图像中的语义内容，区分不同类型的物体和材质，进行针对性处理。'
              )}
            </li>
            <li>
              <strong>{intl.t('更自然的细节重建：')}</strong>
              {intl.t(
                '传统方法通常会产生过度锐化或不自然的纹理，而CodeFormer AI能够生成更符合真实世界的细节。'
              )}
            </li>
            <li>
              <strong>{intl.t('平衡保真度和保真度：')}</strong>
              {intl.t(
                '提供一个"保真度-真实度"的平衡参数，用户可以根据需要调整增强效果，偏向保持原图外观或追求更高质量的重建。'
              )}
            </li>
            <li>
              <strong>{intl.t('适应不同质量的输入：')}</strong>
              {intl.t(
                '对于高分辨率和低分辨率图像均能提供出色的增强效果，适应性更强。'
              )}
            </li>
          </ul>
          ...
          <div className="my-8 p-6 bg-white/5 rounded-xl border border-white/10">
            <h3 className="text-xl font-semibold mb-4">
              {intl.t('CodeFormer AI的主要应用场景')}
            </h3>
            <ul className="space-y-2">
              <li>
                ✅ <strong>{intl.t('照片修复与增强')}</strong>
                {intl.t('：恢复老照片、提升智能手机照片质量')}
              </li>
              <li>
                ✅ <strong>{intl.t('社交媒体图像优化')}</strong>
                {intl.t('：解决微信、小红书等平台的图片压缩问题')}
              </li>
              <li>
                ✅ <strong>{intl.t('人脸照片修复')}</strong>
                {intl.t('：优化低分辨率的人像照片，恢复自然的面部细节')}
              </li>
              <li>
                ✅ <strong>{intl.t('视频帧增强')}</strong>
                {intl.t('：提升视频质量，恢复细节与色彩')}
              </li>
              <li>
                ✅ <strong>{intl.t('图像艺术创作')}</strong>
                {intl.t('：为设计师和摄影师提供高质量图像处理工具')}
              </li>
            </ul>
          </div>
          ...
          <ol>
            <li>
              <strong>{intl.t('朋友圈照片增强')}</strong>
              {intl.t(
                '：将被微信压缩后模糊的照片恢复为清晰自然的高质量图像，显著提升社交媒体上的照片展示效果。'
              )}
            </li>
            <li>
              <strong>{intl.t('老照片修复')}</strong>
              {intl.t(
                '：能够修复褪色、划痕和噪点等问题，同时提高分辨率，使珍贵的老照片焕发新生。'
              )}
            </li>
            <li>
              <strong>{intl.t('人像细节增强')}</strong>
              {intl.t(
                '：特别在面部细节处理上表现出色，能够在保持人物特征的同时，恢复自然的皮肤纹理、眼睛细节等。'
              )}
            </li>
            <li>
              <strong>{intl.t('低光照图像优化')}</strong>
              {intl.t(
                '：能够智能地提亮暗部，同时抑制噪点，改善低光环境下拍摄的照片质量。'
              )}
            </li>
          </ol>

          <h2>{intl.t('CodeFormer AI的未来发展')}</h2>
          <p>
            {intl.t(
          ...
          <ul>
            <li>{intl.t('向更广泛的图像类型扩展，不仅限于人脸图像')}</li>
            <li>{intl.t('实现更高效的处理，降低计算资源需求')}</li>
            <li>{intl.t('增强与其他AI技术的集成，如语义分割和物体识别')}</li>
            <li>{intl.t('开发更多用户友好的应用，使这一技术更加普及')}</li>
          </ul>
          ...
          <div className="mt-12 p-6 bg-white/5 rounded-xl border border-white/10">
            <h3 className="text-xl font-semibold mb-4">
              {intl.t('立即体验CodeFormer AI图像增强')}
            </h3>
            <p className="mb-4">
              {intl.t(
                '想要亲身体验CodeFormer AI的强大功能？我们的在线图片增强工具已经集成了这一技术，现在就可以免费使用！'
              )}
            </p>
            <Link href="/">
